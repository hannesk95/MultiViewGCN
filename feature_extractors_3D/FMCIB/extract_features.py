import sys
sys.path.append('/home/johannes/Data/SSD_1.9TB/MultiViewGCN/3D_feature_extractors/FMCIB/foundation-cancer-image-biomarker')
...
from monai.transforms import (
    ...
    ScaleIntensityd, SpatialPadd, CenterSpatialCropd,
)
...
def extract_features(dataset: str):
    ...
    model = fmcib_model().cuda() if torch.cuda.is_available() else fmcib_model()
    ...
    transforms = Compose([
        ...
        SpatialPadd(keys=["image", "mask"], spatial_size=(50, 50, 50), mode="constant", constant_values=0),
        CenterSpatialCropd(keys=["image", "mask"], roi_size=(50, 50, 50)),
    ...
    for volume, mask in tqdm(zip(volumes, masks)):
        ...
        torch.save(output, f"{volume.replace('.nii.gz', '_FMCIB_features.pt')}")
