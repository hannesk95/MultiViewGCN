...
def extract_features(dataset: str):
    ...
    model = ResEncoder(weights_path="/home/johannes/Data/SSD_1.9TB/MultiViewGCN/feature_extractors_3D/VoCo/checkpoint_final.pth", input_channels=1, num_classes=2).cuda() if torch.cuda.is_available() else ResEncoder(weights_path="/home/johannes/Data/SSD_1.9TB/MultiViewGCN/3D_feature_extractors/VoCo/checkpoint_final.pth", input_channels=1, num_classes=2)
    ...
    for volume, mask in tqdm(zip(volumes, masks)):
        ...
        torch.save(output, f"{volume.replace('.nii.gz', '_VoCo_features_160.pt')}")
