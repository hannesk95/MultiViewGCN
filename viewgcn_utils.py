...
class View_selector(nn.Module):
    def __init__(self, n_views, sampled_view, hidden=384):
        super(View_selector, self).__init__()
        self.n_views = n_views
        self.s_views = sampled_view
        self.hidden = hidden
        self.cls = nn.Sequential(
            nn.Linear(self.hidden*self.s_views, 256*self.s_views),
            nn.LeakyReLU(0.2),
            nn.Linear(256*self.s_views, 2*self.s_views))
    ...
    def forward(self,F,vertices,k):
        ...
        F_score = self.cls(F1).reshape(F.shape[0],k,self.s_views,2).transpose(1,2)
        ...
        F1_id_F = F1_id.unsqueeze(-1).repeat(1, 1, 1, self.hidden)
...
class LocalGCN(nn.Module):
    def __init__(self,k,n_views, hidden=384):
    ...
    def forward(self,F,V):
        F = self.KNN(F, V)
        F = F.view(-1, self.hidden)
        F = self.conv(F)
        F = F.view(-1, self.n_views, self.hidden)
        return F

class NonLocalMP(nn.Module):
    def __init__(self, n_view, hidden=384):
    ...
    def forward(self, F):
        ...
        M = torch.sum(M,-2)
        F = torch.cat((F, M), 2)
        F = F.view(-1, self.hidden * 2)
        F = self.Fusion(F)
        F = F.view(-1, self.n_view, self.hidden)
        return F