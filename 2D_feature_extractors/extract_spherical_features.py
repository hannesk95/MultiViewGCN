...
def extract_spherical_features(dataset, model_name, views):
    ...
    vertices, faces = create_fibonacci_sphere(n_vertices=views, save_sphere=False)
    ...
    for volume, mask in zip(volumes, masks):
        ...
        for img_slice, seg_slice in zip(img_slices, seg_slices):
            ...
            img_pil.save("spherical_output_image.png", format="PNG")
        ...
        torch.save(data, f"{volume.replace('.nii.gz', f'_{model_name}_{str(views).zfill(2)}views_spherical_features.pt')}")
