...
def extract_acs_features(dataset, model_name, views):
    ...
    for volume, mask in tqdm(zip(volumes, masks), total=len(volumes), desc=f"Extracting acs {model_name} features from {dataset} with {views} views"):
        ...
        index_sagittal = find_largest_lesion_slice(seg_tensor, axis=0)
        ...
        match views:
            case 3:
                img_slices.append(img_tensor[index_sagittal, :, :])
            ...
            case _:
                ...
                if views % 3 == 0:
                    ...
                    upper_index_axial = index_axial + ((views // 3) // 2)
                ...
                if views % 3 == 2:
                    ...
                    upper_index_coronal = index_coronal + ((views // 3) // 2) + 1
                    lower_index_coronal = index_coronal - ((views // 3) // 2)
