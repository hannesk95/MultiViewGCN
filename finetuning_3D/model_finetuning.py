...
sys.path.append('/home/johannes/Data/SSD_1.9TB/MultiViewGCN/feature_extractors_3D/SwinUNETR')
...
class SwinUNETR_Classifier(torch.nn.Module):
    def __init__(self, ):
        super(SwinUNETR_Classifier, self).__init__()

        self.swinunetr = ResEncoder(eval=False, weights_path="/home/johannes/Data/SSD_1.9TB/MultiViewGCN/feature_extractors_3D/SwinUNETR/checkpoint_final.pth", input_channels=1, num_classes=2)
        ...
        for i in range(1):
            if i == 0:
                linear = Linear(320, 309)
            else:
                linear = Linear(309, 309)
            
            self.linear_layers.append(linear)
            self.bn_layers.append(BatchNorm1d(309))

        self.linear = torch.nn.Linear(309, 2)
